...
- 成功后卡片变为蓝色已启用状态。
- 不修改 `auth.json`。设为默认是独立操作，写入 `settings.json`。
...
- 提供「设为默认」；删除或移除当前默认供应商时，自动改派到另一个仍启用的节点，或在没有其他启用项时清除。
...
10. 删除或移除当前默认供应商后，`settings.json` 的 defaultProvider 改派到另一个仍启用节点或被清除；`auth.json` 不变。
