...
Parent 决定何时通知 MSI。本审计**不打 tag、不发 MSI、不通知用户换包**。
...
5. **已挂的 v4.1.4 tag / MSI 不含 read 修复。** 若要发包，基于合并 PR #19 之后的 tip，不是 tag `b9c2b694`。

## 给 parent 的一句话

**夹具审计通过；用户本机未验。不要对外说「完整自测通过」。** 合并 PR #19 后再考虑 Windows x64 MSI；通知用户换包之前，至少走完上面 4 下冒烟。
