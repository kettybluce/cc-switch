...
若 MSI 安装失败并提示 **Error 5 / 拒绝访问 `D:\Config.Msi`（或 `%SystemDrive%\Config.Msi`）**，请优先使用绿色版。这是 Windows Installer 回滚缓存的 ACL 问题，与官方相同的 per-user WiX 模板无法在有限权限下修复该目录。说明见 [Windows MSI Error 5](../../../windows-msi-error-5-zh.md)。
