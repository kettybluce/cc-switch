...
- Pi 全局保存的当前供应商和模型由后端读写：卡片「设为默认」写入 `defaultProvider` / `defaultModel`，高亮使用 `piCurrentState.defaultProviderId`。删除或移除当前默认时自动改到另一个仍启用的节点（或清除）。
...
供应商页可以设置并高亮 Pi 的全局默认供应商，写入 `~/.pi/agent/settings.json`。

- 「设为默认」只写 `defaultProvider` 和合理的 `defaultModel`（该节点第一个模型 ID），保留未知字段，不碰 `auth.json`。
- 高亮使用 `piCurrentState.defaultProviderId`，不用 Claude 的 `currentProviderId`。
- 移除或删除当前默认供应商时，同一次写操作把默认改到另一个仍启用的节点；没有其他启用项则清除。确认框仍给出非阻塞提醒。
- `models.json` 状态未知时，启用、移除和删除操作应禁用。全局默认项读取失败时不阻止供应商成员关系操作。

不得用「列表第一个模型」在保存供应商时偷偷修改 Pi 默认选择；只有用户明确点击「设为默认」，或删除/移除当前默认时的自动改派，才会写 `settings.json`。
...
- 状态未知时，不用猜测值恢复写操作。
- 删除和移除必须有确认；目标是 Pi 全局默认供应商时，在同一个确认框内增加非阻塞提醒，并说明会改派或清除默认。
- 保存失败后保留用户输入，显示具体错误。
- 表单错误就地显示并聚焦对应字段。
