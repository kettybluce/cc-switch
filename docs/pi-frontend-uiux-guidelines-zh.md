...
WSL 运行时下有一个界限明确的例外：Pi 运行位置面板可以提供一个默认关闭的开关，把已配置的出站代理注入 Pi 进程的环境变量，并显示 WSL 到 CC Switch 的连通结果。它不产生路由投影、网关状态或故障转移配置，也不改写请求；请求仍由 Pi 自己发出。凭证不得出现在界面、日志、toast 或恢复命令中。详见 [Pi 原生契约与实现边界](./pi-native-contract-zh.md) 的「代理」一节。
