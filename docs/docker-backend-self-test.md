...
| --- | --- |
| `proxy_projection_linux` — POSIX stand-in for Pi / Claude / Codex takeover projection **and** Claude/Codex roundtrips (unknown fields, hot-switch backup, independent disable) | Live WSL UNC (`\\wsl.localhost\…`) on a real Windows host |
