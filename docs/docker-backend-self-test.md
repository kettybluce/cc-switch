...
| --- | --- |
| **Default** `TEST_FILTER=all` — full `cargo test --manifest-path src-tauri/Cargo.toml` (includes `proxy_projection_linux` Pi / Claude / Codex takeover projection, Claude/Codex roundtrips: unknown fields, hot-switch backup, independent disable, fail-closed enable including Pi, Codex OAuth `CodexLiveAuthSwitchGuard` stale-binding cases (`MissingAccount` switch-away / stale target during takeover), **and** `session_usage_scan` isolated-HOME JSONL usage scan) | Live WSL UNC (`\\wsl.localhost\…`) on a real Windows host |
...
- Narrower filters (`proxy` / `session_usage_scan` / `pi-crud` / `lib-lite`) are **not** the shipping report
