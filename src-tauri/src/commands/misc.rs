...
async fn fetch_npm_dist_tags(
    ...
    fetch_npm_dist_tags_at(client, &npm_dist_tags_url(package), LATEST_PROBE_TIMEOUT).await
}

/// 从任意 URL 拉 dist-tags。生产走 registry 专用端点 + 15s 探测超时;测试可注入
/// 本地回环 URL 与更短超时,锁定 404/非法 JSON/挂起 → None,而不打真 npm。
async fn fetch_npm_dist_tags_at(
    client: &reqwest::Client,
    url: &str,
...
mod tests {
    ...
    #[test]
    fn npm_dist_tags_url_encodes_every_slash_and_keeps_scope_at() {
        // Claude / Pi / Gemini 都是 scoped;只转义 `/`,`@` 保留(registry 约定)。
        assert_eq!(
            npm_dist_tags_url("@anthropic-ai/claude-code"),
            "https://registry.npmjs.org/-/package/@anthropic-ai%2fclaude-code/dist-tags"
        );
        assert_eq!(
            npm_dist_tags_url("@earendil-works/pi-coding-agent"),
            "https://registry.npmjs.org/-/package/@earendil-works%2fpi-coding-agent/dist-tags"
        );
        assert_eq!(
            npm_dist_tags_url("@google/gemini-cli"),
            "https://registry.npmjs.org/-/package/@google%2fgemini-cli/dist-tags"
        );
        // 多于一个 `/` 时每一段都转义,不能只替换第一个。
        assert_eq!(
            npm_dist_tags_url("@scope/name/extra"),
            "https://registry.npmjs.org/-/package/@scope%2fname%2fextra/dist-tags"
        );
        // 已是 %2f 的输入不二次编码 `%`。
        assert_eq!(
            npm_dist_tags_url("@openai%2fcodex"),
            "https://registry.npmjs.org/-/package/@openai%2fcodex/dist-tags"
        );
        // 空包名仍拼出合法 URL 形态(请求会 404 → None,不 panic)。
        assert_eq!(
            npm_dist_tags_url(""),
            "https://registry.npmjs.org/-/package//dist-tags"
        );
        // 空格不额外 percent-encode:生产包名无空格;脏输入保持原样由 registry 拒绝。
        assert_eq!(
            npm_dist_tags_url("open claw"),
            "https://registry.npmjs.org/-/package/open claw/dist-tags"
        );
    }

    #[test]
    fn npm_dist_tags_url_matches_npm_package_for_every_known_tool() {
        for tool in [
            "claude", "codex", "gemini", "grok", "opencode", "openclaw", "pi",
        ] {
            let package = npm_package_for(tool).expect(tool);
            let url = npm_dist_tags_url(package);
            assert!(
                url.starts_with("https://registry.npmjs.org/-/package/"),
                "{tool}: {url}"
            );
            assert!(url.ends_with("/dist-tags"), "{tool}: {url}");
            assert!(
                !url.contains("/package//"),
                "empty package segment for {tool}: {url}"
            );
            if package.contains('/') {
                assert!(
                    url.contains("%2f"),
                    "scoped {tool} package {package} must encode slash: {url}"
                );
                assert!(
                    !url.contains(&format!("/package/{package}/")),
                    "raw slash must not remain in {url}"
                );
            }
        }
        assert!(npm_package_for("unknown").is_none());
    }

    #[test]
    fn latest_probe_timeout_is_fifteen_seconds_not_global_client() {
        // 全局转发客户端总超时 600s;探测 latest 必须是请求级 15s,否则卡片「加载中」。
        assert_eq!(LATEST_PROBE_TIMEOUT, std::time::Duration::from_secs(15));
        assert!(LATEST_PROBE_TIMEOUT < std::time::Duration::from_secs(600));
    }

    #[test]
    fn npm_prerelease_tags_only_claude_next() {
        assert_eq!(npm_prerelease_tags("claude"), &["next"]);
        for tool in ["codex", "gemini", "grok", "opencode", "openclaw", "pi", ""] {
            assert!(
                npm_prerelease_tags(tool).is_empty(),
                "{tool} must not probe prerelease tags"
            );
        }
    }
    ...
    #[test]
    fn pick_latest_version_edge_missing_or_dirty_latest() {
        use serde_json::json;
        // 缺 latest / 非字符串 latest → None,调用方显示「未知」。
        assert_eq!(
            pick_latest_version(
                json!({ "next": "2.0.0" }).as_object().unwrap(),
                &["next"],
                Some("2.1.0")
            ),
            None
        ...
        assert_eq!(
            pick_latest_version(json!({ "latest": 1 }).as_object().unwrap(), &["next"], None),
            None
        );
        assert_eq!(
            pick_latest_version(
                json!({ "latest": null, "next": "9.0.0" })
                    .as_object()
                    .unwrap(),
                &["next"],
                Some("9.0.0")
        ...
        // 空字符串 latest 仍是 Some(""),不 panic。
        assert_eq!(
            pick_latest_version(
                json!({ "latest": "" }).as_object().unwrap(),
                &["next"],
                None
            ),
            Some("".to_string())
    ...
    #[test]
    fn pick_latest_version_skips_unparseable_local_and_missing_next() {
        use serde_json::json;
        let tags = json!({
            "latest": "2.1.154",
            "next": "2.1.200"
        ...
        let map = tags.as_object().unwrap();
        // 脏 local / 带 v 前缀无法解析 → 视为未领先,只看 latest。
        assert_eq!(
            pick_latest_version(map, &["next"], Some("false")),
            Some("2.1.154".to_string())
        );
        assert_eq!(
            pick_latest_version(map, &["next"], Some("v2.1.200")),
            Some("2.1.154".to_string())
        );
        // 白名单 tag 不在响应里 → 忽略,仍 latest。
        assert_eq!(
            pick_latest_version(map, &["canary"], Some("2.1.200")),
            Some("2.1.154".to_string())
        );
        // next 是数字 / 空串 / false 脏值 → 比较失败,不替换 latest。
        let dirty = json!({
            "latest": "2.1.154",
            "next": 2,
            "beta": "false",
    ...
    #[test]
    fn pick_latest_version_picks_highest_of_several_prerelease_tags() {
        use serde_json::json;
        let tags = json!({
            "latest": "1.0.0",
            "next": "1.1.0-rc.1",
        ...
        let map = tags.as_object().unwrap();
        // 本地已领先 latest 时,白名单里取能解析且最高者。
        assert_eq!(
            pick_latest_version(map, &["alpha", "beta", "next"], Some("1.0.1")),
            Some("1.1.0-rc.5".to_string())
        );
        // 同核心:预发布 < 正式版,next=1.2.0-rc.1 不会盖过 latest=1.2.0。
        let same_core = json!({ "latest": "1.2.0", "next": "1.2.0-rc.1" });
        assert_eq!(
            pick_latest_version(same_core.as_object().unwrap(), &["next"], Some("1.2.1")),
            Some("1.2.0".to_string())
        );
        // next 等于 latest → 不替换。
        let equal = json!({ "latest": "1.0.0", "next": "1.0.0" });
        assert_eq!(
            pick_latest_version(equal.as_object().unwrap(), &["next"], Some("1.0.1")),
            Some("1.0.0".to_string())
        );
    }

    /// dist-tags HTTP 夹具:本地回环只服务一次,覆盖 200/404/挂起/非法 JSON。
    mod npm_dist_tags_http {
        use super::super::*;
        ...
        fn probe_client() -> reqwest::Client {
            probe_client_with_timeout(Duration::from_secs(2))
        }

        fn probe_client_with_timeout(timeout: Duration) -> reqwest::Client {
        ...
        fn spawn_once_server(
            ...
            (
                format!("http://127.0.0.1:{port}/-/package/@openai%2fcodex/dist-tags"),
                handle,
            )
        }

        #[tokio::test]
        async fn valid_dist_tags_map_is_returned() {
            let body = r#"{"latest":"0.135.0","next":"0.136.0"}"#;
            let (url, handle) = spawn_once_server(Some(http_response("200 OK", body)));
            let tags = fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                .await
                .expect("200 object body");
            assert_eq!(tags.get("latest").and_then(|v| v.as_str()), Some("0.135.0"));
            assert_eq!(tags.get("next").and_then(|v| v.as_str()), Some("0.136.0"));
            let req = handle.join().expect("server").expect("request");
            assert!(
                req.starts_with("GET /-/package/@openai%2fcodex/dist-tags "),
                "must hit injected URL path, got {req}"
            );
            assert!(
                !req.contains("registry.npmjs.org"),
                "must not rewrite to the public registry: {req}"
            );
        }

        #[tokio::test]
        async fn not_found_json_string_body_returns_none() {
            // npm 404 体是 JSON 字符串 `"Not Found"`,不是对象 → 解析 Map 失败 → None。
            let (url, handle) =
                spawn_once_server(Some(http_response("404 Not Found", "\"Not Found\"")));
            assert!(
                fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                    .await
                    .is_none()
            );
        ...
        #[tokio::test]
        async fn array_or_plain_text_body_returns_none() {
            let (url, handle) = spawn_once_server(Some(http_response("200 OK", "[\"latest\"]")));
            assert!(
                fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                    .await
                    .is_none()
            ...
            let (url, handle) = spawn_once_server(Some(http_response("200 OK", "not-json")));
            assert!(
                fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                    .await
                    .is_none()
        ...
        #[tokio::test]
        async fn empty_object_is_some_empty_map() {
            let (url, handle) = spawn_once_server(Some(http_response("200 OK", "{}")));
            let tags = fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                .await
                .expect("empty object is still a Map");
        ...
        async fn connection_refused_returns_none() {
            ...
            let port = listener.local_addr().expect("addr").port();
            drop(listener);
            let url = format!("http://127.0.0.1:{port}/-/package/openclaw/dist-tags");
            assert!(
                fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                    .await
                    .is_none()
        ...
        #[tokio::test]
        async fn hung_response_hits_request_timeout_not_global_600s() {
            // 接受连接后不写响应。探测超时必须在秒级失败，不能吞掉全局 600s。
            let listener = TcpListener::bind("127.0.0.1:0").expect("bind");
            let port = listener.local_addr().expect("addr").port();
            ...
            let result = tokio::time::timeout(
                ...
                fetch_npm_dist_tags_at(&probe_client_with_timeout(timeout), &url, timeout),
            )
            .await
            .expect("request timeout must fire well before the 600s global client");
            assert!(result.is_none(), "hang must become None");
            let elapsed = started.elapsed();
            assert!(
                elapsed < Duration::from_secs(5),
                "must not inherit the 600s global client timeout, elapsed={elapsed:?}"
            );
        }

        #[tokio::test]
        async fn fetch_then_pick_uses_claude_next_when_local_leads() {
            let body = r#"{"latest":"2.1.154","next":"2.1.156","beta":"0.1.1"}"#;
            let (url, handle) = spawn_once_server(Some(http_response("200 OK", body)));
            let tags = fetch_npm_dist_tags_at(&probe_client(), &url, Duration::from_secs(2))
                .await
                .expect("tags");
            assert_eq!(
                pick_latest_version(&tags, npm_prerelease_tags("claude"), Some("2.1.156")),
                Some("2.1.156".to_string())
            );
            assert_eq!(
                pick_latest_version(&tags, npm_prerelease_tags("codex"), Some("2.1.156")),
                Some("2.1.154".to_string())
            );
            handle.join().expect("server");
        }
