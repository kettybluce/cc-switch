...
mod tests {
    ...
    fn pi_openai_provider(id: &str, base_url: &str) -> Provider {
        Provider::with_id(
            id.to_string(),
            id.to_string(),
            json!({
    ...
    async fn start_pi_chat_proxy(
        db: Arc<Database>,
        mock_addr: std::net::SocketAddr,
    ) -> (ProxyServer, crate::proxy::types::ProxyServerInfo) {
        db.save_provider(
            "codex",
            &Provider::with_id(
        ...
        .expect("save dummy codex");
        db.set_current_provider("codex", "codex-current")
            .expect("select dummy codex");
        db.save_provider(
            "pi",
        ...
        let info = proxy.start().await.expect("start test proxy");
        (proxy, info)
    }

    #[tokio::test]
    #[serial_test::serial]
    async fn pi_chat_completions_remaps_developer_role_before_upstream() {
            ...
            move |request: axum::extract::Request| {
                ...
                async move {
                    ...
                    (
                        StatusCode::OK,
                        [(header::CONTENT_TYPE, "application/json")],
                        r#"{"id":"chatcmpl-1","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2}}"#,
                    )
                }
            }
        };
        ...
        let db = Arc::new(Database::memory().expect("memory database"));
        let (proxy, proxy_info) = start_pi_chat_proxy(db, mock_addr).await;
        ...
        let captured = captured.lock().await;
        let request = captured.last().expect("upstream request captured");
        assert_eq!(request.path_and_query, "/v1/chat/completions");
        assert_eq!(request.body["messages"][0]["role"], "system");
        assert_eq!(request.body["messages"][0]["content"], "You are GLM.");
    ...
    #[tokio::test]
    #[serial_test::serial]
    async fn pi_chat_completions_passthrough_upstream_4xx_and_5xx() {
        let _home = TempHome::new();
        let status_cell = Arc::new(Mutex::new(StatusCode::UNAUTHORIZED));
        let handler = {
            let status_cell = status_cell.clone();
            move |_request: axum::extract::Request| {
                let status_cell = status_cell.clone();
                async move {
                    let status = *status_cell.lock().await;
                    let body = match status {
                        StatusCode::UNAUTHORIZED => {
                            r#"{"error":{"message":"invalid api key","type":"auth_error"}}"#
                        }
                        _ => r#"{"error":{"message":"upstream exploded","type":"server_error"}}"#,
                    };
                    (status, [(header::CONTENT_TYPE, "application/json")], body)
                }
            }
        };
        let mock_app = Router::new().route("/v1/chat/completions", post(handler));
        let mock_listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
            .await
            .expect("bind mock upstream");
        let mock_addr = mock_listener.local_addr().expect("mock upstream address");
        ...
        let (proxy, proxy_info) = start_pi_chat_proxy(db, mock_addr).await;
        let client = reqwest::Client::new();
        let url = format!("http://127.0.0.1:{}/v1/chat/completions", proxy_info.port);
        let payload = json!({
            "model": "glm-4",
        ...
        *status_cell.lock().await = StatusCode::INTERNAL_SERVER_ERROR;
        let exploded = client
            .post(&url)
            .header("x-cc-switch-app", "pi")
            .json(&payload)
    ...
    #[tokio::test]
    #[serial_test::serial]
    async fn pi_chat_completions_passthrough_truncated_sse() {
        let _home = TempHome::new();
        let truncated = "data: {\"id\":\"c1\",\"choices\":[{\"delta\":{\"content\":\"hi\"}}]}\n";
        let handler = {
