...
mod tests {
    ...
    #[test]
    fn request_logs_do_not_store_raw_keys_for_claude_and_codex() -> Result<(), AppError> {
        let db = Database::memory()?;
        let logger = UsageLogger::new(&db);
        let key = "sk-ant-api03-TESTSECRETVALUE99xxxx";
        let leaked = format!("上游错误 (401): invalid api_key: {key}");

        for (request_id, app_type) in [("req-claude-key", "claude"), ("req-codex-key", "codex")] {
        ...
        let rows: Vec<(String, String, Option<String>)> = conn
            .prepare(
                "SELECT request_id, app_type, error_message FROM proxy_request_logs
                 WHERE request_id IN ('req-claude-key', 'req-codex-key')
                 ORDER BY request_id",
            )?
        ...
        assert_eq!(rows.len(), 2);
        assert_eq!(crate::database::SCHEMA_VERSION, 19);
