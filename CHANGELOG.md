...
- #56 proxy hot-path hardening (4xx/5xx passthrough, truncated SSE flush, 1214 remap, DeepSeek unwrap) — still open/draft at tag time
- #54 / #49 Docker self-test default `TEST_FILTER=all` + markdown report
- #58 `http_client` / `model_fetch` global-proxy matrix
- #59 WSL UNC / path-normalization regression deepen
...
Ships current `main` after v3.20.13 (tag `7b2bd5c9` / squash `0f3fbcd9`): #50 Docker shipping docs, #51 Claude/Codex takeover roundtrips, #57 frontend Pi tests, #53/#60 fail-closed takeover, #61 secret redaction, #62 i18n/preset lock, #63 SCHEMA 18 guardrails, #55 Portable/release.yml harden. Does **not** include #56 (proxy hot-path hardening still unmerged). Does **not** claim official 3.20.3 / 3.20.14. `SCHEMA_VERSION` stays 18. Default Cursor pool model and fork Pi/WSL/proxy surfaces are preserved. Windows x64 Portable (preferred) + MSI; macOS `.dmg` / `.zip` best-effort.
...
- **#56** proxy/response_processor hot-path hardening (DeepSeek unwrap panic, truncated SSE usage flush, 4xx/5xx mapping, 1214 remap tests, loopback Pi forward) — still open/draft; Windows CI had not finished merging at tag time
- #54 / #49 Docker compose default-full-suite + on-disk report
- #58 model_fetch / global proxy matrix tests
- #59 WSL UNC path-normalization deepen
...
- v3.20.13 users need this for fail-closed takeover, secret redaction, SCHEMA 18 guardrails, and Portable publish harden. Users waiting on proxy panic/SSE/#56 should stay on 3.20.13 or wait for a follow-up.
...
- **会话复制改为 pi --session <会话id>**: 不再复制 Windows UNC / jsonl 文件路径。与官方 Pi `--session <path|id>` 及 Claude/Codex 的 id 命令一致。
