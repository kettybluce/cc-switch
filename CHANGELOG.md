...
- Tray restore after `ExitRequested(None)`: hidden main stays in tray (no longer treated as Cmd+Q / Live-restore quit); missing main after WebView recycle is marked lightweight so 「打开主界面」 recreates the window
- Multi-window: visible secondary window without main follows user-exit Live restore; all-hidden windows stay in tray
- Tray Quit and Unix `SIGTERM`/`SIGINT` share one cleanup sequence (`destroy_single_instance_lock` + once-guard against double Live restore)
...
>>>>>>> 10c8965e (docs: align leftover main notes after Docker full-suite rebase)
